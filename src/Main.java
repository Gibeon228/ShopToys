...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (k != 4) {
            System.out.println("Выберите 1 для добавления игрушки, \n 2 - для розыгрыша, \n 3 - для получения, \n 4 - для выхода из программы\n");
            k = scanner.nextInt();
            switch (k) {
                case (1): {
                    addToys();
                ...
                case (3): {
                    ...
                    if (!prizeToy.isEmpty()) getToy();
                    else System.out.println("Выигрышных игрушек нет");
                    break;
                }
                case (4): {
                    break;
    ...
    private static void playToy() {
        ...
        for (int i = 1; i < toys.size(); i++) {
            if (toys.get(i).getAmount() > 0) temp += toys.get(i).getWeight();
            if (randomValue <= temp) {
    ...
    private static double canTotalWeight() {
        ...
        for (int i = 1; i < toys.size(); i++)
            if (toys.get(i).getAmount() > 0) totalWeight += toys.get(i).getWeight();
        return totalWeight;
    }
